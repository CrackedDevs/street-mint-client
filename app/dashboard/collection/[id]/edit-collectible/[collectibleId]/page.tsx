...
function EditCollectiblePage() {
  ...
  useEffect(() => {
    ...
    const fetchCollectible = async () => {
      ...
      if (fetchedCollectible) {
        const mintStart = fetchedCollectible.mint_start_date
          ? new Date(fetchedCollectible.mint_start_date).toISOString().slice(0, 16)
          : "";

        const mintEnd = fetchedCollectible.mint_end_date
          ? new Date(fetchedCollectible.mint_end_date).toISOString().slice(0, 16)
          : "";
        ...
        setCollectible({
          ...
          mint_start_date: mintStart,
          mint_end_date: mintEnd,
        });
        ...
        setGalleryImages([]);
        fetchedCollectible.gallery_urls?.forEach((url) => {
          fetch(url)
            .then((res) => res.blob())
            .then((blob) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const mintStart = new Date(collectible.mint_start_date ?? "").toISOString();
      const mintEnd = new Date(collectible.mint_end_date ?? "").toISOString();
      ...
      const updatedCollectible: Collectible = {
        ...
        cta_logo_url: collectible.cta_logo_url,
        mint_start_date: mintStart,
        mint_end_date: mintEnd,
      };
        ...
        <Card className="w-full shadow-lg">
          <CardHeader className="space-y-1 pb-8">
            ...
            <CardDescription className="text-center text-lg">
              Edit the details below to edit this collectible.
            </CardDescription>
          </CardHeader>
          <CardContent>
